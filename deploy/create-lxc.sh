#!/usr/bin/env bash
# Create and configure a Spellbook LXC on Proxmox
# Run this on the Proxmox host (not inside the LXC)
# Usage (on Proxmox host):
#   bash create-lxc.sh
#   CT_NAME=spellbook CT_ID=220 bash create-lxc.sh
...
msg "Cloning repo…"
pct exec "$CT_ID" -- rm -rf /tmp/spellbook
pct exec "$CT_ID" -- git clone --branch main https://github.com/RGVylar/spellbook.git /tmp/spellbook
...
pct exec "$CT_ID" -- env \
    ...
    ARCHIMAGO_PASSWORD="$ARCHIMAGO_PASSWORD" \
    bash /tmp/spellbook/deploy/install.sh

echo ""
cat <<EOF

${GRN}✔ Spellbook LXC ready${NC}
...
Update commands (run on Proxmox host):
  # Backend
  pct exec $CT_ID -- bash -c "git config --global --add safe.directory /opt/spellbook && cd /opt/spellbook && git pull && cd backend && .venv/bin/alembic upgrade head && systemctl restart spellbook-backend"

  # Frontend
  pct exec $CT_ID -- bash -c "cd /opt/spellbook/frontend && npm ci && npm run build && systemctl reload caddy"

Logs:
  pct exec $CT_ID -- journalctl -u spellbook-backend -f
